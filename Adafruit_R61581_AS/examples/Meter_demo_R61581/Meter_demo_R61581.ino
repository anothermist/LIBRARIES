/*
 Example animated analogue meters using a R61581 TFT LCD screen
...
{
  // Meter outline
  tft.fillRect(0, 0, 239, 126, R61581_GREY);
  tft.fillRect(5, 3, 230, 119, R61581_WHITE);

  tft.setTextColor(R61581_BLACK);  // Text colour
  ...
  for (int i = -50; i < 51; i += 5) {
    ...
    // Yellow zone limits
    //if (i >= -50 && i < 0) {
    //  tft.fillTriangle(x0, y0, x1, y1, x2, y2, R61581_YELLOW);
    //  tft.fillTriangle(x1, y1, x2, y2, x3, y3, R61581_YELLOW);
    //}

    // Green zone limits
    if (i >= 0 && i < 25) {
      tft.fillTriangle(x0, y0, x1, y1, x2, y2, R61581_GREEN);
      tft.fillTriangle(x1, y1, x2, y2, x3, y3, R61581_GREEN);
    }

    // Orange zone limits
    if (i >= 25 && i < 50) {
      tft.fillTriangle(x0, y0, x1, y1, x2, y2, R61581_ORANGE);
      tft.fillTriangle(x1, y1, x2, y2, x3, y3, R61581_ORANGE);
    }
    ...
    // Draw tick
    tft.drawLine(x0, y0, x1, y1, R61581_BLACK);
...
{
  tft.setTextColor(R61581_BLACK, R61581_WHITE);
  ...
  while (!(value == old_analog)) {
    ...
    tft.drawLine(120 + 20 * ltx - 1, 140 - 20, osx - 1, osy, R61581_WHITE);
    tft.drawLine(120 + 20 * ltx, 140 - 20, osx, osy, R61581_WHITE);
    tft.drawLine(120 + 20 * ltx + 1, 140 - 20, osx + 1, osy, R61581_WHITE);

    // Re-plot text under needle
    tft.setTextColor(R61581_BLACK);
    tft.drawCentreString("%RH", 120, 70, 4); // // Comment out to avoid font 4
    ...
    tft.drawLine(120 + 20 * ltx - 1, 140 - 20, osx - 1, osy, R61581_RED);
    tft.drawLine(120 + 20 * ltx, 140 - 20, osx, osy, R61581_MAGENTA);
    tft.drawLine(120 + 20 * ltx + 1, 140 - 20, osx + 1, osy, R61581_RED);
...
{
  int w = 36;
  tft.drawRect(x, y, w, 155, R61581_GREY);
  tft.fillRect(x+2, y + 19, w-3, 155 - 38, R61581_WHITE);
  tft.setTextColor(R61581_CYAN, R61581_BLACK);
  tft.drawCentreString(label, x + w / 2, y + 2, 2);

  for (int i = 0; i < 110; i += 10)
  {
    tft.drawFastHLine(x + 20, y + 27 + i, 6, R61581_BLACK);
  }

  for (int i = 0; i < 110; i += 50)
  {
    tft.drawFastHLine(x + 20, y + 27 + i, 9, R61581_BLACK);
  }
  
  tft.fillTriangle(x+3, y + 127, x+3+16, y+127, x + 3, y + 127 - 5, R61581_RED);
  tft.fillTriangle(x+3, y + 127, x+3+16, y+127, x + 3, y + 127 + 5, R61581_RED);
...
void plotPointer(void)
{
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    while (!(value[i] == old_value[i])) {
      ...
      {
        tft.drawLine(dx, dy - 5, dx + pw, dy, R61581_WHITE);
        old_value[i]--;
        tft.drawLine(dx, dy + 6, dx + pw, dy + 1, R61581_RED);
      }
      else
      {
        tft.drawLine(dx, dy + 5, dx + pw, dy, R61581_WHITE);
