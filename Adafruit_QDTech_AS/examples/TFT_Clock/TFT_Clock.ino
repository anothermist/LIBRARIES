/*
 ...
 Updated by Alan Senior 5/1/2015
 */
...
void setup(void) {
  ...
  tft.setRotation(0);
  tft.fillScreen(QDTech_GREY);
  tft.setTextColor(QDTech_GREEN, QDTech_GREY);  // Adding a black background colour erases previous text automatically
  ...
  for(int i = 0; i<360; i+= 6) {
    ...
    tft.drawPixel(x0, y0, QDTech_BLUE);
    if(i==0 || i==180) tft.fillCircle(x0, y0, 1, QDTech_CYAN);
    if(i==0 || i==180) tft.fillCircle(x0+1, y0, 1, QDTech_CYAN);
    if(i==90 || i==270) tft.fillCircle(x0, y0, 1, QDTech_CYAN);
    if(i==90 || i==270) tft.fillCircle(x0+1, y0, 1, QDTech_CYAN);
  }
...
void loop() {
  if (targetTime < millis()) {
    ...
    if (ss==0 || initial) {
      ...
      tft.drawLine(ohx, ohy, 65, 65, QDTech_WHITE);
      tft.drawLine(omx, omy, 65, 65, QDTech_WHITE);
      osx = sx*47+65;    
      osy = sy*47+65;
      tft.drawLine(osx, osy, 65, 65, QDTech_RED);
