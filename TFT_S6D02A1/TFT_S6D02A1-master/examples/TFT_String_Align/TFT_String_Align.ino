...
BR_DATUM = Bottom right
 ...
 The library uses the hardware SPI pins only:
   ...
   For Leonardo and 32u4 based Pro Micro:
      MOSI = pin 16, SCK = pin 15
 ...
 Suggested TFT connections for the Leonaro and 32u4 based boards
   sclk 15  // Don't change, this is the hardware SPI SCLK line
   mosi 16  // Don't change, this is the hardware SPI MOSI line
   cs   A2  // TFT chip select line
   dc   A1  // TFT data/command line
   rst  A3  // you could alternatively connect this to the Arduino reset
...
void loop() {
  ...
  for(byte datum = 0; datum < 9; datum++) {
    ...
    tft.drawNumber(88,80,64,6);
    tft.fillCircle(80,64,5,TFT_RED);
    
    tft.setTextDatum(MC_DATUM);
    
    tft.setTextColor(TFT_BLACK);
    tft.drawString("X",80,64,2);
    delay(1000);
    tft.fillScreen(TFT_BLACK);
  }

  tft.setTextColor(TFT_BLUE, TFT_BLACK);
  tft.drawCentreString("69",80,64,6);
  tft.fillCircle(80,64,5,TFT_YELLOW);
  ...
  tft.drawRightString("88",80,64,6);
  tft.fillCircle(80,64,5,TFT_YELLOW);
  ...
  float test = 67.125;
  tft.drawFloat(test, 4, 80, 80, 4);
  delay(1000);
  tft.fillScreen(TFT_BLACK);
  test = -0.555555;
  tft.drawFloat(test, 3, 80, 80, 4);
  delay(1000);
  tft.fillScreen(TFT_BLACK);
  test = 0.1;
  tft.drawFloat(test, 4, 80, 80, 4);
  ...
  tft.fillScreen(TFT_BLACK);
  test = 9999999;
  tft.drawFloat(test, 1, 80, 80, 4);
  delay(1000);
  
  tft.fillCircle(80,80,5,TFT_YELLOW);
  
  tft.setTextDatum(MC_DATUM);
  
  tft.setTextColor(TFT_BLACK);
  tft.drawString("X",80,80,2);
