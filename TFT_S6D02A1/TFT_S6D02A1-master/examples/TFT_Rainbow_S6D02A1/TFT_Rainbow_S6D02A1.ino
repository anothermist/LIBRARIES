...
void loop() {
  ...
  if (targetTime < millis()) {
    ...
    tft.setTextColor(S6D02A1_BLACK, S6D02A1_BLACK); // Note these fonts do not plot the background colour
    tft.setCursor (12, 5);
    tft.print("Original ADAfruit font!");
    
    // The new larger fonts do not use the .setCursor call, coords are embedded
    tft.setTextColor(S6D02A1_BLACK, S6D02A1_BLACK); // Do not plot the background colour
